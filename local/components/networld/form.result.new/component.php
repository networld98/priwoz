	...
	if ($arResult["ERROR"] == '')
	{
		...
		*/
		...
		if (!empty($_REQUEST["formresult"]) && $_REQUEST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'])
		{
			...
			{
				case 'ADDOK':
                   if($APPLICATION->GetCurPage() != SITE_DIR."community/"){
                       $arResult['FORM_NOTE'] = str_replace("#RESULT_ID#", $RESULT_ID, GetMessage('FORM_NOTE_ADDOK'));
                   }else{
                       $arResult['FORM_NOTE'] = str_replace("#RESULT_ID#", $RESULT_ID, GetMessage('FORM_NOTE_ADDOK_COMM'));
                   }
			}
		}
		...
		*/
		...
		foreach ($arResult["arQuestions"] as $key => $arQuestion)
		{
				...
				foreach ($arResult["arAnswers"][$FIELD_SID] as $key => $arAnswer)
				{
					...
					switch ($arAnswer["FIELD_TYPE"])
					{
						case "radio":
                            ...
                            $input = "<input class='form-check-input' id='".$ans_id."' type='radio' data-value='".$arAnswer["VALUE"]."' value='".$ans_id."' name='form_radio_".$FIELD_SID."'>";
						...
						case "textarea":
                            ...
                            if($arParams['COMPONENT_TEMPLATE']=='custom'){
                                $res = "<div class='form-group other-reason-textarea' style='display: none;'><textarea name='form_textarea_".$arAnswer["ID"]."' class='form-control' placeholder='Введите текст до 300 символов с пробелами'></textarea></div>";
                            }else{
						...
						case "image":
                           ...
                           // $res = '<div class="upload-file-custom"><div class="upload-image-box"><input name="form_file_'.$arAnswer["ID"].'" id="form_file_'.$arAnswer["ID"].'" class="typefile" size="20" type="file" accept=".png, .jpg, .jpeg, .pdf"><label for="form_file_'.$arAnswer["ID"].'" id="customFileInputLabel" class="upload-image -default" data-default="'.SITE_TEMPLATE_PATH.'/images/icons/upload-file.svg"  data-pdf="'.SITE_TEMPLATE_PATH.'/images/icons/PDF_Logo.svg"><img id="previewImage" class="preview-image" src="'.SITE_TEMPLATE_PATH.'/images/icons/upload-file.svg" alt="Preview"></label> <div class="hint">'.GetMessage("FORM_FILE_SIZE").'</div></div></div>';
			...
			$arResult["QUESTIONS"][$FIELD_SID]["HTML_CODE"] = implode("", $arResult["QUESTIONS"][$FIELD_SID]["HTML_CODE"]);
