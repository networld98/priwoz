$( document ).ready(function() {
	...
	if(categoryVal != '' && subCategoryVal == '' ) {
		...
		$(".filter-output .count span").text($('.ads-item').length);
...
$(document).ajaxComplete(function() {
	...
	if(categoryVal == '' && subCategoryVal == ''){
		$(".page-title").text('Доска объявлений');
		$(".filter-output-box").hide('fast');
	}else if(categoryVal != '' && subCategoryVal == '' ) {
		$(".page-title").text(category);
		$(".filter-output .count span").text($('.ads-item').length);
		$(".filter-output-box").show('fast');
	}else if(subCategoryVal != '' && subCategoryVal != 'Все') {
		$(".page-title").text(subCategory);
		$(".filter-output .count span").text($('.ads-item').length);
		$(".filter-output-box").show('fast');
	}
...
JCSmartFilter.prototype.values2post = function (values)
{
	...
	{
		if(values[i].name!='ajax' && values[i].name!='q' && values[i].name!='smartPreFilter_529') {
			url[i] = values[i].name + '=' + values[i].value;
		}
		if(values[i].name=='q'){
		...
		if(values[i].name=='arrFilter_527'){
			var categoryBread = 'Y';
			var categoryUrl = values[i].value;
		}
