...
$res = \Neti\Favorite\Entity\PriceChangeFavoriteTable::getList([
	'select' => ['UF_PRODUCT_ID']
])->fetchAll();
$productsId = [];
foreach ($res as $el) {
	$productsId[] = $el['UF_PRODUCT_ID'];
}

$res = \Neti\Favorite\Entity\PriceChangeFavoriteTable::getList([
    'filter' => ['UF_PRODUCT_ID' => array_keys($arResult['ITEMS'])]
])->fetchAll();

foreach ($arResult['ITEMS'] as $id => $product) {
	if (in_array($id, $productsId) === false) {
		foreach ($arResult['ITEMS'][$id]['OFFERS'] as $offersId) {
			if (in_array($offersId['ID'], $productsId)) {
				$res = \Neti\Favorite\Entity\PriceChangeFavoriteTable::getList([
					'select' => ['UF_OLD_PRICE'],
					'filter' => ['UF_PRODUCT_ID' =>['UF_PRODUCT_ID' => $offersId['ID']]]
				])->fetch();
				break;
			}
		}
		...
		$res = \Neti\Favorite\Entity\PriceChangeFavoriteTable::getList([
			'select' => ['UF_OLD_PRICE'],
			'filter' => ['UF_PRODUCT_ID' =>['UF_PRODUCT_ID' => $id]]
		])->fetch();
	}

    $currency = preg_replace('/^[0-9]*[.,][0-9]+/', '', $product['MIN_PRICE']['PRINT_DISCOUNT_VALUE']);
    $oldPrice = number_format($res['UF_OLD_PRICE'], 2, '.', ' ');
	$arResult['ITEMS'][$id]['PRICE_OLD'] = $oldPrice .  $currency;
}
?>